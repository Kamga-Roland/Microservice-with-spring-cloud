...
public class GrpcServerConfig {
    ...
    /**
     * Crée et démarre manuellement le serveur gRPC de tp9-server (Port 9091)
     */
    @Bean
    ...
    /**
     * Crée le client (Stub) dynamique pour appeler tp9-stock
     */
    @Bean
    public StockServiceGrpc.StockServiceBlockingStub stockServiceBlockingStub(DiscoveryClient discoveryClient) {
        List<ServiceInstance> instances = discoveryClient.getInstances("tp9-stock");

        if (instances == null || instances.isEmpty()) {
            throw new IllegalStateException("Aucune instance du service 'tp9-stock' n'a été trouvée dans Eureka !");
        }
        ...
        System.out.println("[gRPC Client] Connexion vers tp9-stock établie sur " + host + ":" + webPort);
